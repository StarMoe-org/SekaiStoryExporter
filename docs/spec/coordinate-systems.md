...
| `UnityScreen` | 屏幕**左下** | ↑ 向上 | px | UGUI 的 screen space；**游戏的 UI 参数都在这个系** |
...
理由：所有来自游戏的参数（RectTransform 数值、CanvasScaler 结果、坐标表）天然就在这个系里。选它做中枢意味着**游戏数据可以零转换地直接使用**，减少一整类抄写错误。
...
| 表示 | 类型 | 地位 |
|---|---|---|
| **模拟帧号** | `SimFrame(u32)` | ⭐ **权威表示**。时间轴、参数表以它索引；步长 = 游戏帧率 60（ADR-0009） |
| **导出帧号** | `FrameNo(u32)` | 渲染、哈希、diff 的索引；60 fps 导出时与模拟帧一一对应（ADR-0009） |
...
2. **`sse-timeline` 按帧模拟**（ADR-0009）：每模拟帧 `elapsed += deltaTime`（f32，与 Unity 一致），以 `elapsed < duration` 决定是否再等一帧。
